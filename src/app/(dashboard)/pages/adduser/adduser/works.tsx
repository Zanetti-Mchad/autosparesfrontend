...
const AddUser = () => {
  ...
  const sendWelcomeSMS = async (phone: string, name: string, email: string, password: string, userId: string) => {
    try {
      // Check if SMS credentials are available
      const username = process.env.NEXT_PUBLIC_EGOSMS_USERNAME;
      const password = process.env.NEXT_PUBLIC_EGOSMS_PASSWORD;
      const sender = process.env.NEXT_PUBLIC_EGOSMS_SENDER;
      ...
      const welcomeMessage = `Welcome ${name} to AutoSpares! Your login email is ${email}, your login phone number is ${phone}. Your password is ${password}.`;
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let photoUrl = '';
      if (photoFile) {
        // In a real implementation, you would upload the photo to your storage
      ...
      if (formData.phone) {
        try {
          const smsResult = await sendWelcomeSMS(
            ...
            formData.email,
            formData.password || '',
            createdUser.id // Use id from wrapped data
          );
          ...
          toast('User created but failed to send welcome SMS', {
            position: 'top-center',
            duration: 3000,
            icon: '⚠️',
            style: {
