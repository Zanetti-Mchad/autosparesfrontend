...
const ReportsView = () => {
  ...
  const [orders, setOrders] = useState<Array<{
    ...
    date?: string;
    items?: Array<{ product: string; size?: string; quantity: number }>;
  }>>([]);
  ...
  const handleExportFilteredOrders = (list: RecentOrder[]) => {
    const headers = ['Order', 'Customer', 'Items', 'Total', 'Status', 'Payment Status', 'Date'];
    const rows = list.map(o => {
      const itemsStr = Array.isArray(o.items) && o.items.length
        ? o.items.map(it => `${it.product}${it.size ? ` (${it.size})` : ''} × ${it.quantity}`).join('; ')
        : '';
      return [
        o.orderNumber || o.id,
        o.customer || '',
  ...
  const loadInventory = useCallback(async () => {
    try {
      ...
      console.log('[Reports][Inventory] Fetch inventory ->', { url, status: res.status, ok: res.ok });
      if (!res.ok) throw new Error(`Failed to fetch inventory: ${res.status}`);
      const result = await res.json();
      console.log('[Reports][Inventory] Raw JSON:', result);
      ...
      console.log('[Reports][Inventory] Mapped items count:', mapped.length);
    ...
    return (
      <div className="p-6 border-b border-border/50">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={handleExportInventoryList} className="p-2 glass rounded-lg hover:bg-secondary transition-colors flex items-center gap-2" title="Export Inventory List">
              <Download className="w-4 h-4" />
            </button>
          </div>
        </div>
