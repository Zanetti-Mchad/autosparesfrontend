...
const ProfilePage = () => {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      // External backend: single correct endpoint and payload
      // Omit Authorization for this public endpoint
      ...
      const requestBody: Record<string, unknown> = {
        ...
        ...(isEmail ? { baseUrl: origin } : {})
      };

      // Debug: log where OTP is being sent and to whom
      console.log('[OTP REQUEST] Sending OTP via', isEmail ? 'email' : 'phone', 'to:', verificationContact, 'with baseUrl:', isEmail ? origin : undefined);
      ...
      console.log('🔁 OTP request', { url, requestBody, status: res.status, ok: res.ok, data });
      // Attempt to extract OTP from common response shapes
      const receivedOtp = (data && (
        data.otp ||
        (data.data && (data.data.otp || data.data.code)) ||
        data.code ||
        (data.result && (data.result.otp || data.result.code)) ||
        (data.payload && (data.payload.otp || data.payload.code))
      )) || undefined;
      if (receivedOtp) {
        console.log('[OTP RECEIVED FROM BACKEND]', receivedOtp);
      } else {
        console.log('[OTP RECEIVED FROM BACKEND] No explicit OTP field found in response. Full payload logged above.');
      }
      if (!res.ok || (data.status && data.status.returnCode !== '00')) {
        throw new Error(data.message || (data.status && data.status.returnMessage) || `Failed to send verification code (${res.status})`);
      }

      setOtpSent(true);
      showMessage('Verification code sent successfully! Please check your ' + (verificationMethod === 'email' ? 'email' : 'phone') + ' for the code.');
    } catch (err: unknown) {
            ...
            {activeTab === 'security' && (
              <div className="p-8 bg-gradient-to-br from-gray-50 to-blue-50">
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="md:col-span-2">
                      <input
                        ...
                        placeholder={verificationMethod === 'email' ? 'Enter your email' : 'Enter your phone number'}
                    ...
                    <div className="md:col-span-1">
                      <button
                        ...
                        className="w-full bg-blue-600 text-white px-4 py-3 rounded-xl hover:bg-blue-700 disabled:opacity-50"
                      >
                  ...
                  <form onSubmit={handleOtpResetPassword} className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                      ...
                      <div className="md:col-span-3">
                        <button
                          type="submit"
                          disabled={passwordLoading}
                          className="w-full bg-green-600 text-white px-4 py-3 rounded-xl hover:bg-green-700 disabled:opacity-50"
                        >
