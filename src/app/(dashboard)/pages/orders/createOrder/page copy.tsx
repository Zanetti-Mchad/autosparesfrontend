...
const CreateOrder = () => {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        ...
        const formattedCustomers = data.map((customer: ApiCustomer) => ({
          id: customer.id,
          ...
          shippingDistrict: customer.shippingDistrict || customer.district || '',
    ...
    const fetchInventory = async () => {
      try {
        ...
        const mockInventory: InventoryItem[] = [
          { id: 1, name: 'iPhone 14 Pro Max', price: 1200, category: 'Electronics', size: '', stock: 3, description: 'Latest Apple iPhone', photo: '' },
          { id: 2, name: 'Wireless Headphones', price: 250, category: 'Accessories', size: '', stock: 12, description: 'Noise cancelling', photo: '' },
          { id: 3, name: 'T-Shirt', price: 25, category: 'Clothing', size: 'M', stock: 45, description: 'Cotton t-shirt', photo: '' },
        ];
  ...
  const handleProductSelect = (itemId: string, productId: string) => {
    const selectedProduct = inventory.find(item => item.id === Number(productId));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const orderData = {
      ...
      total: orderItems.reduce((sum, item) => {
        return sum + (parseFloat(item.sellingPrice) || 0) * (item.quantity || 0);
      }, 0)
    };
