...
const OrderModal: React.FC<OrderModalProps> = ({ order, mode, onClose, onSave }) => {
  ...
  const computeItemTotal = (item: OrderItem): number => {
    return toNumber(item.price) * (item.quantity || 0);
  };

  const recomputeOrderTotals = (ord: Order): Order => {
    const newSubtotal = ord.items.reduce((sum, it) => sum + computeItemTotal(it), 0);
    const vatNum = toNumber(ord.vat);
    const newTotal = newSubtotal + vatNum;
    return {
      ...ord,
  ...
  const updateItem = (itemId: string, patch: Partial<OrderItem>) => {
    setDraft(prev => {
      if (!prev) return prev;
      const updatedItems = prev.items.map(it => it.id === itemId ? { ...it, ...patch } : it);
      return recomputeOrderTotals({ ...prev, items: updatedItems });
    });
  };
                    ...
                    <td className="p-3 text-right">
                      {mode === 'edit' ? (
                        <input
                          ...
                          value={toNumber(it.price)}
                          onChange={(e) => updateItem(it.id, { price: String(e.target.value) })}
                        />
                      ) : (
                        formatAmount(it.price)
                      )}
                    </td>
                    <td className="p-3 text-right">{formatAmount(computeItemTotal(it))}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  const updateOrder = async (updated: Order) => {
    ...
    const mappedItems = (updated.items || [])
      .map(it => ({
        ...
        size: it.size,
        sellingPrice: toNumber(it.price),
      }))
      .filter(it => {
        const ok = Boolean(it.inventoryId && String(it.inventoryId).trim().length > 0);
        if (!ok) {
          try { console.warn('Skipping item without inventoryId in update payload:', it); } catch {}
        }
        return ok;
      });

    if (!mappedItems.length) {
      throw new Error('No valid items to update. Each item must have a valid inventoryId.');
    }

    const payload = {
      // Intentionally omit status to avoid unintended changes
      items: mappedItems,
    };
  ...
  const fetchOrderById = async (orderId: string): Promise<Order | null> => {
    try {
      ...
      const mappedItems = rawItems.map((it: any) => ({
        ...
        inventoryId: String(it.inventoryId ?? it.inventory?.id ?? it.inventory_id ?? ''),
        ...
        if (Array.isArray(items)) {
          ...
          const mapped: Order[] = items.map((o: any) => {
            ...
            const mappedItems = rawItems.map((it: any, idx: number) => {
              ...
              const chosenInventoryId = it.inventoryId ?? it.inventory?.id ?? it.inventory_id ?? undefined;
              ...
              console.log('Item mapping → id:', chosenId,
                ...
                '| inventoryId:', chosenInventoryId || 'N/A'
              );

              return {
                id: chosenId,
                inventoryId: chosenInventoryId ? String(chosenInventoryId) : undefined,
                product: chosenName,
