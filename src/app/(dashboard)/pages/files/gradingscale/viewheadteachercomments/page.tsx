...
const ViewHeadTeacherComments: React.FC = () => {
  const [comments, setComments] = useState<HeadTeacherComment[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedComment, setSelectedComment] = useState<HeadTeacherComment | null>(null);
...
// This is a focused update for the fetchComments function in your ViewHeadTeacherComments component
...
const fetchComments = useCallback(async () => {
  try {
    ...
    console.log(`Fetching comments from: ${env.BACKEND_API_URL}/api/v1/headteacherscomments/comments?${queryParams.toString()}`);

    const response = await fetch(`${env.BACKEND_API_URL}/api/v1/headteacherscomments/comments?${queryParams.toString()}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      }
    ...
    const groupedComments = rawComments.reduce((acc: HeadTeacherComment[], comment: any) => {
      const existingGroup = acc.find((g: HeadTeacherComment) => 
      ...
      if (existingGroup) {
        ...
        // Create new group
        const newComment: HeadTeacherComment = {
          id: comment.id,
  ...
  const handleView = (comment: HeadTeacherComment) => {
    setSelectedComment(comment);
    setViewMode('detail');
  };

  // Handler for editing a comment set
  const handleEdit = (comment: HeadTeacherComment) => {
    setSelectedComment(comment);
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await fetch(`${env.BACKEND_API_URL}/api/v1/headteacherscomments/update-comments/${selectedComment.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleDelete = async (commentId: string) => {
    try {
      ...
      const response = await fetch(`${env.BACKEND_API_URL}/api/v1/headteacherscomments/comments/${commentId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const formatUserInfo = useCallback((comment: HeadTeacherComment): string => {
      ...
      <div className="min-h-screen bg-gray-100 p-6">
        <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-lg overflow-hidden">
          ...
          <div className="bg-green-600 text-white p-4">
            ...
            <h1 className="text-2xl font-bold text-center">Head Teacher Comments</h1>
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-lg overflow-hidden">
        ...
        <div className="bg-blue-600 text-white p-4">
          ...
          <h1 className="text-2xl font-bold text-center">Edit Head Teacher Comments</h1>
          <h2 className="text-xl font-semibold text-center">
            {selectedComment.class ? `${selectedComment.class.name}${selectedComment.class.section ? ` - ${selectedComment.class.section}` : ''}` : 'Unknown Class'}
          </h2>
        </div>
      ...
      <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-lg overflow-hidden">
        ...
        <div className="bg-green-600 text-white p-4">
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-2xl font-bold">Head Teacher Comments</h1>
            <Link
              href={{
                pathname: '/pages/gradingscale/headteachercomments',
                query: {
...
export default ViewHeadTeacherComments;