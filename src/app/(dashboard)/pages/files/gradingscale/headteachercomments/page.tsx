...
interface CommentRowWithRange {
  startMarks: number;
  endMarks: number;
  comment: string;
}

function computeRanges(rows: CommentRow[]): CommentRowWithRange[] {
  const sorted = [...rows].sort((a, b) => Number(a.endMarks) - Number(b.endMarks));
  const result: CommentRowWithRange[] = [];
  for (let i = 0; i < sorted.length; i++) {
    const startMarks = Number(sorted[i].endMarks);
    let endMarks: number;
    if (i < sorted.length - 1) {
      endMarks = Number(sorted[i + 1].endMarks) - 1;
    } else {
      endMarks = Number(sorted[i].endMarks);
    }
    result.push({ startMarks, endMarks, comment: sorted[i].comment });
  }
  return result;
}
...
const HeadTeacherCommentsContent: React.FC<{}> = () => {
...
const fetchExistingComments = useCallback(async () => {
  ...
  try {
    ...
    const response = await fetch(`${env.BACKEND_API_URL}/api/v1/headteacherscomments/comments?academicYearId=${currentYear.id}&termId=${currentTerm.id}&classId=${classId}`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
...
const validateAndSaveComments = useCallback(async () => {
  try {
    ...
    const response = await fetch(`${env.BACKEND_API_URL}/api/v1/headteacherscomments/create-comments`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
   ...
   <div className="min-h-screen bg-gray-100 flex items-center justify-center">
     <div className="container mx-auto bg-white shadow-lg rounded-lg p-6 max-w-xl">
       ...
       <h1 className="text-2xl font-bold text-gray-800 text-center mb-6">
         Head Teacher Comments - Grading Scale
       </h1>
...
const HeadTeacherCommentsPage: React.FC = () => {
  return (
    <React.Suspense fallback={<div>Loading head teacher comments...</div>}>
      <HeadTeacherCommentsContent />
    </React.Suspense>
  );
};

export default HeadTeacherCommentsPage;