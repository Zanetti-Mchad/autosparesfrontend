...
const TimeTable: React.FC = () => {
  ...
  useEffect(() => {
    const fetchExistingTimetable = async () => {
      if (!selectedSection || displayClasses.length === 0) return;
      try {
        // Build query string for class IDs
        const classIds = displayClasses.map(c => c.id).join(',');
        // Adjust the endpoint as needed for your backend
        const response = await authFetch(`${baseUrl}/timetable/filter?section=${selectedSection}&classId=${classIds}`);
        if (!response.ok) throw new Error('Failed to fetch timetable');
        const data = await response.json();
        if (data.success && data.timetables && data.timetables.length > 0) {
          // Use the first timetable returned
          const timetable = data.timetables[0];
          // Map entries to timetableData
          const mappedData: TimetableData = {};
          timetable.entries.forEach((entry: any) => {
            // Find class name for the entry
            const className = displayClasses.find(c => c.id === entry.classId)?.name || "";
            const key = [entry.day, entry.timeSlot, entry.classId, className].join('|||');
            mappedData[key] = entry.subjectActivityId;
          });
          setTimetableData(mappedData);
          // Optionally, restore mergedCells if you store that info in the backend
        } else {
          // If no timetable found, clear timetableData
          setTimetableData({});
        }
      } catch (error) {
  ...
  }, [selectedSection, displayClasses]);
  ...
  const handleSave = async () => {
    ...
    const timetablePayload = {
      name: "Primary School Timetable 2025",
      academicYearId: "a5d47709-708d-491d-9b32-38460038dd0c",
      termId: "1a8f48da-39da-491f-989b-e20cb66be31c",
      specialPeriods: [
        { timeSlot: "7:30am-8:00am", name: "MORNING TEA", bgColor: "bg-blue-200", lightBgColor: "bg-blue-100" },
