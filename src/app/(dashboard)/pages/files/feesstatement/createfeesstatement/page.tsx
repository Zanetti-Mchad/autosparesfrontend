          ...
          <div className="hidden md:grid md:grid-cols-2 gap-4 mb-6 lg:hidden">
            {filteredStudents.map((student, index) => (
              <div
                ...
                className="bg-white rounded-lg shadow border border-gray-200 p-4 flex flex-col justify-between cursor-pointer hover:bg-gray-50"
                onClick={() => openDialog(student)}
              >
                <div className="mb-2 font-semibold text-gray-900">{student.name}</div>
                <div className="mb-2 text-xs text-gray-700">School Pay Code: <span className="font-medium">{student.schoolPayCode}</span></div>
                <div className="mb-2 text-xs text-gray-700">Bursary: <span className="font-medium">{student.bursaryStatus || 'No'}</span></div>
                <div className="mb-2 text-xs text-gray-700">Balance Prev. Term: <span className="font-medium">{formatNumber(student.balancePrevTerm)}</span></div>
                <div className="mb-2 text-xs text-gray-700">Fees Payable: <span className="font-medium">{formatNumber(student.feesPayable)}</span></div>
                <div className="mb-2 text-xs text-gray-700">Stationery: <span className="font-medium">{formatNumber(student.stationery)}</span></div>
                <div className="mb-2 text-xs text-gray-700">Total Expected: <span className="font-medium">{formatNumber(student.totalExpected)}</span></div>
                <div className="mb-2 text-xs text-gray-700">Fees Paid: <span className="font-medium">{formatNumber(student.feesPaid)}</span></div>
                <div className="mb-2 text-xs text-gray-700">Balance: <span className="font-medium">{formatNumber(student.balance)}</span></div>
                <div className="mb-2 text-xs text-gray-700">Comments: <span className="font-medium">{student.comments}</span></div>
              </div>
            ))}
          </div>
          ...
          <div className="md:hidden space-y-4 mb-6">
            {filteredStudents.map((student, index) => (
              <div
                ...
                onClick={() => openDialog(student)}
              >
                <div className="mb-2 font-bold text-base text-gray-800">{student.name}</div>
                <div className="mb-2 text-xs text-gray-700">School Pay Code: <span className="font-medium">{student.schoolPayCode}</span></div>
                <div className="mb-2 text-xs text-gray-700">Bursary: <span className="font-medium">{student.bursaryStatus || 'No'}</span></div>
                <div className="mb-2 text-xs text-gray-700">Balance Prev. Term: <span className="font-medium">{formatNumber(student.balancePrevTerm)}</span></div>
                <div className="mb-2 text-xs text-gray-700">Fees Payable: <span className="font-medium">{formatNumber(student.feesPayable)}</span></div>
