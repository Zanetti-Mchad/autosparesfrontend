  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Normalize assessment type to expected API values
        const lat = (urlAssessmentType || '').toLowerCase();
        const normalizedAssessmentType = lat.includes('mid') ? 'MID'
          : (lat.includes('end') || lat.includes('eot')) ? 'EOT'
          : (lat.includes('beg') || lat.includes('bot')) ? 'BOT'
          : lat.includes('test') ? 'TEST'
          : (urlAssessmentType ? urlAssessmentType.toUpperCase() : 'EOT');
        
        if (!classId || classId === 'defaultClassId') {
          throw new Error('Missing required parameter: classId');
        }
        
        // For this report we only consider the selected assessment type
        const selectedAssessmentType = normalizedAssessmentType || 'EOT';
        
        const headers = getAuthHeaders();
        
        // Fetch class-assigned subjects directly and map nested subjectActivity
        const classSubjectsUrl = `${baseUrl}/class-subject-assignments/class/${classId}`;
        const classSubjectsRes = await fetch(classSubjectsUrl, { headers });
        if (!classSubjectsRes.ok) throw new Error(`Subjects for class fetch failed: ${classSubjectsRes.status}`);
        const classSubjectsData = await classSubjectsRes.json();
        let fetchedAssignedSubjects: Subject[] = [];
        if (classSubjectsData.status?.returnCode === '00' && Array.isArray(classSubjectsData.data?.assignments)) {
          const raw = classSubjectsData.data.assignments
            .map((a: any) => a.subjectActivity)
            .filter((sa: any) => sa && sa.id && sa.name);
          const uniq = new Map<string, Subject>();
          raw.forEach((sa: any) => {
            const s: Subject = { id: sa.id, code: sa.code || sa.name.substring(0,3).toUpperCase(), name: sa.name };
            if (!uniq.has(s.id)) uniq.set(s.id, s);
          });
          fetchedAssignedSubjects = Array.from(uniq.values()).sort((a, b) => a.name.localeCompare(b.name));
        }
        ...
        await Promise.all(fetchedAssignedSubjects.map(async (subject) => {
          const url = `${baseUrl}/marks/get?subjectId=${subject.id}&classId=${classId}&termId=${termId}&academicYearId=${academicYearId}&examSetId=${examSetId}&assessmentType=${selectedAssessmentType}`;
          try {
            ...
            if (!response.ok) {
              console.warn(`Marks fetch failed for ${subject.name} with ${selectedAssessmentType}: ${response.status}`);
              return;
            }
            
            const data = await response.json();
            if (!data || data.status?.returnCode !== "00" || !data.data?.marks) {
              // No valid data; leave marks as null for this subject
              return;
            }
            ...
            marksList.forEach((markEntry: Mark) => {
              ...
              // Store mark value (including -1) or set null if missing
              if (markValue !== null) {
                studentRecord.marks[subject.code] = markValue;
                studentRecord.assessmentTypeUsed[subject.code] = selectedAssessmentType;
              } else if (!(subject.code in studentRecord.marks)) {
                studentRecord.marks[subject.code] = null;
                studentRecord.assessmentTypeUsed[subject.code] = null;
              }
            });
          } catch (error: any) {
            if (error.name === 'AbortError') console.warn(`Request timeout for ${subject.name} with ${selectedAssessmentType}`);
            else console.error(`Error fetching ${subject.name} with ${selectedAssessmentType}:`, error);
          }
        }));
