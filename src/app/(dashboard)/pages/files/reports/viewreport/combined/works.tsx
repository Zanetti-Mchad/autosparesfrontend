    ...
    {schoolInfo.loading ? <div className="text-center py-8 text-gray-500">Loading school info...</div> : 
     schoolInfo.error ? <div className="text-center py-8 text-red-600">{schoolInfo.error}</div> : (
      <div className="flex justify-between items-start gap-2">
        <div className="w-28 h-28 md:w-36 md:h-36 border border-gray-600 relative">
          {schoolInfo.badge ? (
            <Image 
...
const ReportsPageContent = () => {
  ...
  const fetchTeacherAssignments = useCallback(async (classId?: string): Promise<Map<string, string>> => {
    const map = new Map<string,string>();
    if(!classId) return map;
    try {
      const res = await fetch(`${env.BACKEND_API_URL}/api/v1/teacher-subject-assignments/assignments?classId=${classId}`,{headers:getAuthHeaders()});
  ...
  const fetchAllMarks = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        fetchTeacherAssignments(classId), 
