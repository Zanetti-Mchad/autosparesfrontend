...
const AnalysisSheetView: React.FC = () => {
      ...
      try {
        ...
        if (classSubData.status?.returnCode === '00' && Array.isArray(classSubData.data?.assignments)) {
          ...
          rawSubjects.forEach((sa: any) => {
            const s: Subject = { 
              id: sa.id, 
              code: sa.code || sa.name.substring(0,3).toUpperCase(), 
              name: sa.name,
              isCompulsory: sa.isCompulsory || false
            };
            if (!unique.has(s.id)) unique.set(s.id, s);
          });
          // Sort subjects with compulsory subjects first, then optional subjects
          assignedSubjects = Array.from(unique.values()).sort((a, b) => {
            // First sort by compulsory status
  ...
  const studentsWithTotals = useMemo(() => {
    return students.map(student => {
      ...
      let hasAnyMissedCompulsory = false;

      subjects.forEach(subject => {
        const subjectScoreData = student.scores[subject.code];
        const isCompulsory = subject.isCompulsory === true;
        
        if (subjectScoreData) {
          hasAnyScore = true;
          if (subjectScoreData.grade === 'X') {
            hasAnyMissedPaper = true;
            if (isCompulsory) hasAnyMissedCompulsory = true;
          } else if (typeof subjectScoreData.grade === 'number') {
            // Only add to total if it's a compulsory subject
            if (isCompulsory) {
      ...
      if (hasAnyMissedCompulsory) {
        // If any compulsory subject is missed, mark as X regardless of other scores
        totalAggregateForDisplay = 'X';
        division = 'X';
      } else if (subjectsAttemptedCount > 0) {
        totalAggregateForDisplay = numericTotalAggregate;
        // Updated division ranges per requirements:
        // 4-12 = I, 13-24 = II, 25-28 = III, 29-32 = IV, 33-36 = U
        if (numericTotalAggregate >= 4 && numericTotalAggregate <= 12) division = 'I';
        else if (numericTotalAggregate >= 13 && numericTotalAggregate <= 24) division = 'II';
        else if (numericTotalAggregate >= 25 && numericTotalAggregate <= 28) division = 'III';
        else if (numericTotalAggregate >= 29 && numericTotalAggregate <= 32) division = 'IV';
        else if (numericTotalAggregate >= 33) division = 'U';
        else division = 'U'; // Default for any other cases
      } else if (!hasAnyScore) {
 ...
 // Calculate DIV 1% as percentage of students with D1, D2, or C3 grades
      const topGradesCount = gradesCount.D1 + gradesCount.D2 + gradesCount.C3;
      const div1Percent = studentsWithNumericGrade > 0 ? (topGradesCount / studentsWithNumericGrade * 100).toFixed(1) : '0.0';
  ...
  return (
    <PrintableContent title="" className="p-4 sm:p-6 max-w-7xl mx-auto bg-white shadow-lg rounded-md print:shadow-none print:bg-transparent">
      ...
      <div className="mb-6 print:mb-3">
        ...
        <div className="text-xs mb-2"><span className="text-red-600 font-bold text-sm">*</span> Only <strong>Compulsory subjects</strong> are added in <strong>TOTAL AGGREGATE</strong> calculation and are denoted with <span className="text-red-600 font-bold text-sm">*</span> </div>  
        <div className="overflow-x-auto">
          <table className="w-full border-collapse border border-gray-400 text-xs sm:text-sm print:text-xs">
            <thead>
              <tr className="bg-gray-100 print:bg-gray-100">
                ...
                {subjects.map(subject => (
                  <React.Fragment key={subject.code}>
                    <th className="border p-1 sm:p-2 border-gray-400">
                      {subject.code.toUpperCase()}
                      {subject.isCompulsory && <span className="text-red-600 text-base font-bold">*</span>}
                    </th>
            ...
            <tbody>
              {studentsWithTotals
                .sort((a, b) => {
                  // Special handling for 'X' values - they should be at the bottom
                  if (a.totalAggregate === 'X' && b.totalAggregate !== 'X') return 1;
                  if (a.totalAggregate !== 'X' && b.totalAggregate === 'X') return -1;
                  
                  // Handle other non-numeric values like '-'
                  if (typeof a.totalAggregate !== 'number' && typeof b.totalAggregate === 'number') return 1;
                  if (typeof a.totalAggregate === 'number' && typeof b.totalAggregate !== 'number') return -1;
                  
                  // If both are numbers, sort normally
                  if (typeof a.totalAggregate === 'number' && typeof b.totalAggregate === 'number') {
                    return a.totalAggregate - b.totalAggregate; // Sort from lowest to highest
                  }
                  
                  // If both are non-numeric and not 'X', keep original order
                  return 0;
                })
                .map((student, index) => {
                return (
