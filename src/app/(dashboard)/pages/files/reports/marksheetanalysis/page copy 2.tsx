...
const AnalysisSheetView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const assignmentsResponse = await fetch(assignmentsUrl, { headers: getAuthHeaders() });
        if (!assignmentsResponse.ok) throw new Error(`Assignments fetch failed: ${assignmentsResponse.status}`);
        const assignmentsData = await assignmentsResponse.json();
        const classSubjectIds: string[] = (assignmentsData.data?.assignments || [])
        ...
        const subjectsResponse = await fetch(subjectsUrl, { headers: getAuthHeaders() });
        ...
        const assignedSubjects: Subject[] = allSubjects
        ...
        for (const subject of assignedSubjects) {
          const url = `${baseUrl}/marks/get?subjectId=${subject.id}&classId=${classId}&termId=${termId}&academicYearId=${academicYearId}&examSetId=${examSetId}&assessmentType=${urlAssessmentType}`;
  ...
  const userName = user.first_name && user.last_name ? `${user.first_name} ${user.last_name}` : 'Unknown User';
  ...
  return (
    <PrintableContent title="" className="p-4 sm:p-6 max-w-7xl mx-auto bg-white shadow-lg rounded-md print:shadow-none print:bg-transparent">
      ...
      <div className="mt-8 print:mt-4 no-print">
        <div className="border-t pt-4 space-y-4">
          <div className="space-y-2">
            <p className="font-bold text-md sm:text-lg print:text-base">CLASS TEACHER&apos;S COMMENT:</p>
            <div className="border p-3 bg-gray-50 rounded print:border-none print:bg-transparent">
              <textarea
                value={teacherComment}
                onChange={(e) => setTeacherComment(e.target.value)}
                className="w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-green-500 print:border-gray-300"
                rows={3}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4 font-bold text-xs sm:text-sm print:text-xs">
