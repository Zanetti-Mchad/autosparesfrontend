  ...
  const studentsWithTotals = useMemo(() => {
    return students.map(student => {
      ...
      if (subjectsAttemptedCount > 0) {
        ...
        if (hasAnyMissedCompulsory) division = 'X';
      } else if (hasAnyMissedCompulsory && subjectsAttemptedCount === 0) {
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse border border-gray-400 text-xs sm:text-sm print:text-xs">
            ...
            <tbody>
              {studentsWithTotals
                .sort((a, b) => {
                  // Handle non-numeric values like 'X' or '-'
                  const aTotal = typeof a.totalAggregate === 'number' ? a.totalAggregate : Infinity;
                  const bTotal = typeof b.totalAggregate === 'number' ? b.totalAggregate : Infinity;
                  return aTotal - bTotal; // Sort from lowest to highest
                })
                .map((student, index) => {
                return (
