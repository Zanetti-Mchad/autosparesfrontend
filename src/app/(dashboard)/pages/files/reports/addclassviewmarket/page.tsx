...
interface ExamSet {
  ...
  classId: string;
  termId: string;
  examSetId?: string; // Add this property to store the actual examSetId
}
...
const MarksheetGenerator: React.FC = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        // Fetch subjects
        const subjectsResponse = await fetch(`${baseUrl}/subjects/filter?limit=100`, { headers });
  ...
  useEffect(() => {
    const fetchExamSets = async () => {
      ...
      try {
        ...
        // Add this log to see the full response structure
  ...
  const handleYearChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // Find the selected academic year and update terms
    const selectedAcademicYear = academicYears.find(year => year.year === selectedYearValue);
    if (selectedAcademicYear && selectedAcademicYear.terms) {
      setTerms(selectedAcademicYear.terms);
      // Reset term selection to first available term
      if (selectedAcademicYear.terms.length > 0) {
        setSelectedTerm(selectedAcademicYear.terms[0].name);
      } else {
        setSelectedTerm("");
      }
    } else {
    ...
    // Reset exam selection when year changes
    setSelectedExam("");
  };

  const handleTermChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setSelectedTerm(e.target.value);
    // Reset exam selection when term changes
    setSelectedExam("");
  };
  ...
  const shouldShowSubject = () => {
    return selectedExam === 'Continuous Assessment (C.A)';
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // Prepare base query parameters with the correct ID
    const baseParams = {
      class: encodeURIComponent(classValue),
      exam: encodeURIComponent(examValue),
      year: encodeURIComponent(yearValue),
      term: encodeURIComponent(termValue),
      classId: selectedClassObj?.id,
      examSetId: actualExamSetId,  // Use the actual exam set ID
      termId: selectedTermObj?.id,
      academicYearId: selectedYearObj?.id
    };
    ...
    // Log the final URL
    const finalUrl = `/pages/reports/marksheet/viewmarksheet?${queryParams.toString()}`;
    ...
    if (examValue === 'Continuous Assessment (C.A)') {
      ...
      const selectedSubjectObj = subjects.find(subject => subject.name === subjectValue);

      // Add subject parameters
      queryParams.append('subject', encodeURIComponent(subjectValue));
      if (selectedSubjectObj?.id) {
        queryParams.append('subjectId', selectedSubjectObj.id);
      }

      // Navigate to CA marksheet view
      router.push(`/pages/reports/marksheet/viewca?${queryParams.toString()}`);
    } else {
      // Navigate to standard marksheet view
    ...
    <div className="bg-gray-100 min-h-screen pt-8">
      <div className="bg-white shadow-md rounded-lg p-6 w-full max-w-md mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              >
                <option value="" disabled>
                  {terms.length === 0 ? "No terms available" : "Select Term"}
                </option>
                {terms.map(term => (
          ...
          {/* Subject Selector for Continuous Assessment */}
          {shouldShowSubject() && (
            <div>
