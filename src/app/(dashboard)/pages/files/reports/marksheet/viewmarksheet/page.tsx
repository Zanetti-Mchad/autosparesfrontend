  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch assigned subjects for this specific class using the class-focused endpoint
        // This returns assignments with nested subjectActivity objects whose IDs match subjectActivityId
        const classAssignmentsUrl = `${env.BACKEND_API_URL}/api/v1/class-subject-assignments/class/${classId}`;
        const classAssignmentsResponse = await fetch(classAssignmentsUrl, { headers });
        if (!classAssignmentsResponse.ok) {
          throw new Error(`Subjects for class fetch failed: ${classAssignmentsResponse.status}`);
        }

        const classAssignmentsData = await classAssignmentsResponse.json();

        // Map assignments -> Subject[] using nested subjectActivity
        let assignedSubjects: Subject[] = [];
        if (classAssignmentsData.status?.returnCode === "00" && Array.isArray(classAssignmentsData.data?.assignments)) {
          const rawSubjects = classAssignmentsData.data.assignments
            .map((assignment: any) => assignment.subjectActivity)
            .filter((sa: any) => sa && sa.id && sa.name);

          // Optional: filter by provided termId/academicYearId if present on assignment
          // Keep only unique subjects by id and sort by name
          const uniqueById = new Map<string, Subject>();
          rawSubjects.forEach((sa: any) => {
            ...
            if (!uniqueById.has(subjectItem.id)) {
              uniqueById.set(subjectItem.id, subjectItem);
            }
          });
          assignedSubjects = Array.from(uniqueById.values()).sort((a, b) => a.name.localeCompare(b.name));
        }
