...
import { Search, ChevronLeft, ChevronRight, Edit, Save, X, Calendar, ArrowLeft, AlertCircle, Check, Printer, RotateCw } from 'lucide-react';
...
const StudentManagement = () => {
      ...
      <Card className="shadow-lg">
        ...
        <CardContent className="overflow-hidden">
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-16 edit-column">
                      Edit
                    </th>
                  </tr>
                </thead>
                  ...
                  {loading ? (
                    ...
                    students.map((student: Student, index: number) => (
                      ...
                      >
                        ...
                        <td className="px-2 py-4 whitespace-nowrap text-sm text-gray-500 edit-column">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            className="flex items-center p-1 bg-blue-500 text-white rounded hover:bg-blue-600"
                            title="Edit Student"
                          >
          ...
          {pagination && students.length > 0 && (
            <div className="flex items-center justify-between mt-4 px-2 py-3 no-print">
  ...
  const StudentEditForm = () => {
    ...
    const [showStationaryDialog, setShowStationaryDialog] = useState(false);
    const [pendingClassChange, setPendingClassChange] = useState<{value: string, stationaryAmount: string} | null>(null);
    ...
    const handleSubmit = async (formData: any, photoFile: File | null) => {
      ...
      try {
        ...
        if (photoFile && formData.student_photo && formData.student_photo !== selectedStudent.student_photo) {
          console.log("Would upload new photo:", photoFile.name);
        }
    ...
    // Handle class change with stationary dialog
    const handleClassChange = (value: string) => {
      ...
      setShowStationaryDialog(true);
      
      return false; // Prevent immediate form update
    };

    const handleConfirmStationaryUpdate = () => {
      if (!pendingClassChange || !formRef.current) return;
      
      // Apply the stationary update and class change
      formRef.current.updateFormData({
        class_assigned: pendingClassChange.value,
        stationary_amount: pendingClassChange.stationaryAmount
      });
      
      // Close dialog
      setShowStationaryDialog(false);
      setPendingClassChange(null);
    };

    const handleCancelStationaryUpdate = () => {
      if (!pendingClassChange || !formRef.current) return;
      
      // Just update class without changing stationary amount
      formRef.current.updateFormData({
        class_assigned: pendingClassChange.value
      });
    ...
    const initialData = {
      ...
      class_assigned: selectedStudent.class_assigned || '',
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
        ...
        {/* Stationary Dialog */}
        {showStationaryDialog && pendingClassChange && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1000]">
            <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full">
              <h3 className="text-lg font-medium mb-2">Update Stationary Amount</h3>
              <p className="mb-4">
                Would you like to update the stationary amount to 
                <span className="font-semibold"> {pendingClassChange.stationaryAmount} </span> 
              ...
              <div className="flex justify-end space-x-3">
                <button 
                  ...
                  onClick={handleCancelStationaryUpdate}
                >
                  No, Keep Current
                </button>
                <button 
                  ...
                  onClick={handleConfirmStationaryUpdate}
                >
                  Yes, Update
                </button>
              </div>
