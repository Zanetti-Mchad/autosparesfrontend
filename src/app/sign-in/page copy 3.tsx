...
const Login = () => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (currentView === 'signin') {
      ...
      try {
        ...
        const isEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.forgotEmail || '');
        const identifierType = isEmail ? 'email' : 'phone';
        
        console.log('Sending password reset request to:', `${process.env.NEXT_PUBLIC_API_URL}/auth/request-password-reset`);
        
        // Format phone number if needed
        let identifier = data.forgotEmail || '';
        console.log('Original input:', identifier);
        ...
        if (!isEmail) {
          ...
          if (digits.startsWith('0')) {
            identifier = `256${digits.substring(1)}`;  // Replace leading 0 with 256
          } else if (!digits.startsWith('256')) {
            identifier = `256${digits}`;  // Add 256 if no country code
          } else {
            identifier = digits;  // Already has +256
          }
          
          // Ensure we have a valid phone number length (12 digits with country code)
          if (identifier.length !== 12) {
            console.error('Invalid phone number length:', identifier.length, 'digits');
            throw new Error('Please enter a valid phone number (e.g., 07XXXXXXXX)');
          }
          
          console.log('Formatted phone number:', identifier);
        }
        
        const requestBody: any = {
          // For OTP requests, use the original phone number format (0782651854)
          // The 256 format is only for sending SMS
          identifier: isEmail ? identifier : data.forgotEmail,
          type: identifierType
        };
        ...
        // Add baseUrl for email reset if it's an email
        if (isEmail) {
          requestBody.baseUrl = window.location.origin; // Current frontend URL
        }
        ...
        if (responseData.data?.otp) {
          console.log('OTP received from backend:', responseData.data.otp);
        }
        
        if (!response.ok) {
          throw new Error(responseData.status?.returnMessage || 'Failed to send verification code');
        }
        ...
        if (responseData.status?.returnCode === 200) {
          ...
          localStorage.setItem('otpData', JSON.stringify({
            identifier: data.forgotEmail,
            type: identifierType,
            otp: responseData.data?.otp, // OTP: ${responseData.data?.otp}
            expiresAt: new Date(Date.now() + (responseData.data?.expiresIn * 1000 || 10 * 60 * 1000)).toISOString()
          }));
          
          setCurrentView('otp');
          startCountdown();
        } else {
          setErrorMessage(responseData.status?.returnMessage || 'Failed to send verification code');
        }
      } catch (error) {
  ...
  const goBack = () => {
    resetForm();
    setCurrentView(prev => {
      if (prev === 'otp') return 'forgot';
      if (prev === 'reset' || prev === 'success') return 'otp';
      return 'signin';
    });
  };
